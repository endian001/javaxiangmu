...
class Refuse extends RowAction
{
        ...
        try {
            $result = DB::transaction(function () use ($id, $ip, $ua) {
                ...
                UserOperateLog::insertLog(
                    ...
                    $ip,
                    '',
                    $this->shortText('admin '.$this->adminName().' refused withdraw id='.$model->id.' amount='.$model->amount.' balance='.$beforeBalance.'->'.$user->balance),
                    $this->auditInfo([
                        'action' => 'withdraw_refuse',
    ...
    {
        return OperationPermission::can(OperationPermission::FINANCE_WITHDRAW_REFUSE, $user);
    }
