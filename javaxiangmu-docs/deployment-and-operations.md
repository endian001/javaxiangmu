...
- 工单、内部实时客服、消息、公告。
- Dcat Admin 用户、权限、菜单。
- TCG 平台设置、游戏管理、KYC、推广、积分商城、玩家运营和运营记录。
- `system_config` 运行时配置。
...
- 支付商户配置正确。
- 客服链接、内部实时客服、工单或 Stream Chat 配置可用。
...
- `internal_live_chat_enabled` 是否开启。
- 实时客服会话表和消息表是否已迁移。
- 前台客服 payload 是否返回内部实时客服状态。
- 游客 visitor id 是否符合长度和字符规则。
- 后台在线客服路由是否在 Dcat Admin 登录态下可访问。
- 消息轮询是否被前置网关、CORS 或限流阻断。
...
- 内部实时客服游客入口不依赖登录态，应增加限流、内容治理和异常 visitor id 监控。
...
- 游戏服务、支付服务和客服配置读取。
- public 前台入口。
- 内部实时客服迁移、前台页面、API 控制器、后台控制器和测试。
