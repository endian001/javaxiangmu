...
- 活动分类的 `name` 更偏后台中文管理名。
- 活动分类的 `enname` 用作前台优先展示名。
- 活动主体的 `entitle`、`encontent`、`enmemo` 是前台优先展示文案。
- 接口保留 `admin_name`，避免后台运营识别分类时丢失中文语义。
...
- 运营可对特定用户或活动加限制。
- 活动券支持指定用户或通用券。
- 翻倍规则支持全局或指定活动，并可按金额区间和有效期选择优先规则。
- 服务在表不存在时降级，便于增量上线。
...
- 当前证据只确认翻倍规则选择服务和后台表，不足以确认奖励发放闭环已经完整接入。
...
- 活动黑名单表不存在时会降级。
- 活动素材和多语言文案需要统一管理。
- 活动翻倍规则如果只配置未接入发奖链路，可能造成运营预期和实际效果不一致。
